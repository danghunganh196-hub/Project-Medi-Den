...
<div class="order-status-page">
    ...
    <div class="timeline-wrapper">
        ...
        <c:choose>
            <c:when test="${donHang.trangThai == 'Chờ xác nhận'}">  <c:set var="step" value="1"/></c:when>
            <c:when test="${donHang.trangThai == 'Đang xử lý'}">    <c:set var="step" value="2"/></c:when>
            <c:when test="${donHang.trangThai == 'Đang giao'}">     <c:set var="step" value="3"/></c:when>
            <c:when test="${donHang.trangThai == 'Hoàn thành'}">    <c:set var="step" value="4"/></c:when>
        </c:choose>
